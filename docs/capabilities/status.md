...
|---|---:|---|
| `kernel-tested` | 1 | Deterministic domain capability with offline contract evidence, not exposed to an Agent. |
...
Current Agent surface: 12 `finance_*` tools, 9 canonical Skills, the `skill` loader, one `finance-analyst` preset, and headless/Web profiles. Neither status nor a directory name implies live provider availability.
