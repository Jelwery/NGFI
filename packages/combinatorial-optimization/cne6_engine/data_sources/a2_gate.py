...
def audit_a2(root: Path, candidate: Path, contract_path: Path, repo_root: Path) -> dict:
    ...
    gates.extend([
        ...
        {"id": "riskModelFeasibility", "status": risk_probe["status"], "scope": "24-security-sample",
         "maxCrossSectionSize": risk_probe.get("maxCrossSectionSize"),
         "feasibleFullRankDays": risk_probe.get("feasibleFullRankDays"),
         "observedTradedDays": risk_probe.get("observedTradedDays"), "reason": risk_probe.get("reason")},
        {"id": "riskAcceptance", "status": "blocked", "scope": "full-market",
         "reason": "No accepted full-market 42-descriptor/risk coverage and covariance calibration; the sample cross-section is rank-deficient (N<=K), so no covariance can be estimated or accepted at sample scope either."},
