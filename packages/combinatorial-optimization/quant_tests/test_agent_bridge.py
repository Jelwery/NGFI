...
class AgentBridgeTest(unittest.TestCase):
    ...
    def test_optimizer_fixed_whitelist_and_json_contract(self):
        from test_optimizer import example_input
        for operation in ("portfolio-optimize", "rebalance-plan"):
            result = self.run_bridge(operation, example_input())
            self.assertEqual(result.returncode, 0, result.stderr)
            output = json.loads(result.stdout)
            self.assertEqual(output["status"], "ok", output)
            self.assertEqual(output["operation"], operation)
            self.assertIn("cashDelta", output["repaired"]["trades"][0])
        result = self.run_bridge("bash", {"command": "touch forbidden"})
        self.assertEqual(result.returncode, 2)
        self.assertNotIn("Traceback", result.stderr)
        duplicate = subprocess.run([sys.executable, "-m", "ngfi_quant.agent_bridge", "portfolio-optimize"],
                                   input='{"dryRun":true,"dryRun":false}', text=True, capture_output=True)
