...
def calc_all(data):
    ...
    for ys in years:
        ...
        res["solvency"][ys] = {"asset_liability_ratio":sd(tl,ta),"equity_multiplier":sd(ta,pe) if pe else None,
            "total_debt":st+lt if st is not None and lt is not None else None,"st_borrowing":st,"lt_borrowing":lt,"cash_to_st_debt":sd(cash,st) if st else None,
    ...
    for ys in years:
        ...
        cpx = gv(annual,y,"现金流量表","购建固定资产、无形资产和其他长期资产所支付的现金")
        rev = gv(annual,y,"利润表","营业收入")
        fcf_val = ocf - abs(cpx) if ocf is not None and cpx is not None else None
    ...
    for ys in years:
        ...
        if debt is None:
            direction = "N/A (负债数据不足)"
        elif debt == 0:
            direction = "N/A (无有息负债)"
        elif r_est is None or sp is None:
            direction = "N/A (利率或ROA数据不足)"
        else:
