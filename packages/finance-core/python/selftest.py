...
extract_path = Path(__file__).resolve().parents[2] / "finance-data-service/providers/astock/python/sources/company_financials.py"
extract_spec = importlib.util.spec_from_file_location("company_financials", extract_path)
extract_module = importlib.util.module_from_spec(extract_spec)
extract_spec.loader.exec_module(extract_module)
format_stock_code = extract_module.format_stock_code
...
check("未知负债不可声称无有息负债",
      r["roa_vs_r"]["2024"]["leverage_direction"] == "N/A (负债数据不足)")
...
check("derive_depr 无数据保持缺失", dep is None and src == "missing")

nwc = cv.derive_nwc_rate(d["annual_data"], years_sorted)
check("derive_nwc_rate 无收入增量无法估计", nwc is None, f"got {nwc}")
...
for scenario in scen.values():
    scenario.update(admin_rate=0.035, rd_rate=0.045, other_inc_rate=0.01,
                    tax_surcharge_rate=0, tax_rate=0.15, depr_annual=10,
                    capex_annual=50, nwc_change_rate=0.04, depr_in_cogs=False,
                    net_debt=-200, minority_ratio=0.02, total_shares=10)
    scenario["assumption_sources"] = {key: "explicit synthetic fixture" for key in scenario}
...
assumptions = dcf_synth["neutral"]["quality"]["assumptions"]
check("税率来自显式假设", assumptions["tax_rate"]["value"] == 0.15)
check("资本开支来自显式假设", assumptions["capex_annual"]["value"] == 50)
check("少数股东比例来自显式假设", assumptions["minority_ratio"]["value"] == 0.02)
missing_scen = json.loads(json.dumps(scen))
del missing_scen["neutral"]["tax_rate"]
check("缺税率拒绝自动估值", "missing explicit" in cv.calc_dcf(d, missing_scen)["error"])
...
check("RIM 留存率字段缺失保持未知",
      rim_ok["retention_source"] == "missing" and rim_ok["payout_ratio"] is None)
...
check("RIM 衰减无留存假设拒绝", cv.calc_rim(d_fade, 0.09, 0.025).get("status") == "needs_input")
rim_fade = cv.calc_rim(d_fade, 0.09, 0.025, payout_ratio=0.5)
...
for y in d_div["annual_data"]:
    ...
    d_div["annual_data"][y]["现金流量表"]["子公司支付给少数股东的股利、利润"] = 0.0
    d_div["annual_data"][y]["利润表"]["利息支出"] = 0.0
