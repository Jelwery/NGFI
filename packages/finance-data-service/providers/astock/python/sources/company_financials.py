...
def format_stock_code(code):
    ...
    match = re.fullmatch(r"(?:(sh|sz)\.?){0,1}([0-9]{6})", str(code).strip().lower())
    if not match:
        raise ValueError(f"无效股票代码: {code!r}（应为6位数字，如 600519 或 sh.600519）")
    prefix, c = match.groups()
    exchange = "sh" if c.startswith(("6", "9")) else "sz" if c.startswith(("0", "3")) else None
    if exchange is None or (prefix and prefix != exchange):
        raise ValueError(f"不支持或交易所冲突的代码: {code!r}")
    return f"{exchange}{c}"
...
def extract_all_data(stock_code, years=5, verbose=True):
    ...
    return company_type, annual, latest_qtr, prev_year_qtr, {
        ...
        "provider": "akshare",
        "upstream": "sina",
        "source_kind": "vendor-normalized",
        "retrieved_at": datetime.now(timezone.utc).isoformat(),
        "currency": "CNY",
        "amount_unit": "yuan",
        "publication_time": None,
        "available_at": None,
        "pit": {"status": "unavailable", "reason": "Sina snapshot does not establish historical availability or restatement vintages"},
        "status": "partial" if errors else "ok",
        "limitations": ["Vendor API is not exchange-direct evidence; reconcile material values with filings.", "Blank cells remain null. Retrieval time is not publication/availability time."],
        "latest_quarter_period": str(latest_qtr_period) if latest_qtr_period is not None else None,
        "prev_year_quarter_period": str(int(str(latest_qtr_period)) - 10000) if prev_year_qtr else None,
...
def extract_single_period(data, period):
    ...
    for table_name, df in data.items():
        ...
        for col in df.columns:
            ...
            val = row[col].values[0]
            if pd.isna(val):
                row_dict[col] = None  # Provider blanks are unknown, not confirmed zeros.
                continue
            try:
                num_val = float(val)
    ...
    for period in sorted(annual_periods):
        ...
        for table_name, df in data.items():
            ...
            for col in df.columns:
                ...
                val = row[col].values[0]
                if pd.isna(val):
                    row_dict[col] = None  # Preserve unknown cash/debt and other missing fields.
                    continue
                try:
                    num_val = float(val)
                    row_dict[col] = num_val
...
def print_summary(company_type, annual_data, metadata):
    ...
    print("\n" + "=" * 70)
    print("数据来源: akshare / Sina vendor-normalized（非交易所直连，PIT可得时间未知）")
    print("需交叉验证: 归母净利润、EPS、总资产、营收（官方财报或冻结审计文件）")
    print("=" * 70)
