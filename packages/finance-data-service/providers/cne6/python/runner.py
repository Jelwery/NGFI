...
def validate_artifacts(data_root: Any) -> ArtifactContext:
  ...
  provenance = {
    ...
    "quality": {
      "quality_flag": "proxy",
      "coverage": coverage,
      "reasons": ["current_cap_and_industry_not_historical_pit", "annual_fundamentals_not_mrq_ttm", "announcement_date_origin_not_verified", "annual_dividend_proxy"],
    },
    "partial": partial,
    "failed": failed,
    "requested": requested,
...
def query_artifact(context: ArtifactContext, params: Any, max_rows: int) -> dict[str, Any]:
  ...
  if dataset == "fundamentals" and as_of is not None:
    pit_grade = "partial"
    pit_reason = "available_date filtering applied; announcement-date origin and annual/dividend proxies are not verified PIT"
